%% Generate caldera points
% Load and plot LVC data
LVCnodes = 'caldera_nodes.csv';
caldera_UTM = csvread(LVCnodes, 2, 0);
caldera = zeros(length(caldera_UTM(:,1)), 2);

% Convert from UTM
for i = 1:length(caldera_UTM(:,1))
    [caldera(i,1), caldera(i,2)] = utm2deg(caldera_UTM(i,1), caldera_UTM(i,2), '11 N');
end

csvwrite('caldera_points.csv', caldera)

%% Generate resurgent dome points
% Load and plot RD data
RDnodes = 'RD_nodes.csv';
RD_UTM = csvread(RDnodes, 2, 0);
RD = zeros(length(RD_UTM(:,1)), 2);

% Convert from UTM
for i = 1:length(RD_UTM(:,1))
    [RD(i,1), RD(i,2)] = utm2deg(RD_UTM(i,1), RD_UTM(i,2), '11 N');
end

csvwrite('RD_points.csv', RD)

%% Generate Hilton Creek Fault  points
% Load and plot HCF data
HCF_nodes = 'HCF_nodes.csv';
HCF_UTM = csvread(HCF_nodes, 2, 0);
HCF = zeros(length(HCF_UTM(:,1)), 2);

% Convert from UTM
for i = 1:length(HCF_UTM(:,1))
    [HCF(i,1), HCF(i,2)] = utm2deg(HCF_UTM(i,1), HCF_UTM(i,2), '11 N');
end

csvwrite('HCF_points.csv', HCF)
