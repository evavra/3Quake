function plotTimeMag(catalog, numTicks)
...
hold on
plot(numDates, catalog{:, 5}, 'k.', 'markersize', 10)


%for i = 1:length(dates)
%    plot(dates(i) * ones(length(datesEQ{i}),1), catalog{datesEQ{i}, 5}, 'k.', 'markersize', 10)
%end

grid on

xTicks = linspace(floor(numDates(1)),ceil(numDates(end)), numTicks);
xLabels = datestr(xTicks, 'mm/dd/yyyy');

axis([xTicks(1) xTicks(end) 0 max(catalog{:,5} + 0.5)])
xlabel('Date')
xticks(xTicks)
xticklabels(xLabels)

ylabel('Magnitude')
yticks(0:0.5:max(catalog{:,5} + 0.5))
